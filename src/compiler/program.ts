namespace ts {
    ...
    export function createProgram(rootNamesOrOptions: readonly string[] | CreateProgramOptions, _options?: CompilerOptions, _host?: CompilerHost, _oldProgram?: Program, _configFileParsingDiagnostics?: readonly Diagnostic[]): Program {
        ...
        function emitWorker(program: Program, sourceFile: SourceFile | undefined, writeFileCallback: WriteFileCallback | undefined, cancellationToken: CancellationToken | undefined, emitOnlyDtsFiles?: boolean, customTransformers?: CustomTransformers, forceDtsEmit?: boolean): EmitResult {
            ...
            // @ts-expect-error
            if (options.transformers && !global['ts-plus']?.resolvingTransformers) {
                // @ts-expect-error
                ...
                for (const entry of options.transformers) {
                    ...
                    if (!factory) {
                        throw new Error(`tsconfig.json > plugins: "${entry.name}" does not have export "default": ` + require("util").inspect(factoryModule));
                    }
                    if (typeof factory !== "function") {
                        throw new Error(`tsconfig.json > plugins: "${entry.name} export "default" is not a plugin: ` + require("util").inspect(factory));
                    }
                    const inst = factory(program, cleanConfig);
                    if (typeof inst === "function") {
                        if (position && position === 'after') {
                            afterTransformers.push(inst);
                        }
                        else if (position && position === 'afterDeclaration') {
                            afterDeclarationsTransformers.push(inst);
                        }
                        else {
                            beforeTransformers.push(inst);
                        }
                    } else {
                        beforeTransformers.push(...inst.before);
                        afterTransformers.push(...inst.after);
                        afterDeclarationsTransformers.push(...inst.afterDeclarations);
                    }
                }
            }
